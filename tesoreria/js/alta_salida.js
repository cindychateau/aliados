$(document).ready(function(){
	...
	$(document).on('change','#grupo',function(e) {
		...
		$.ajax({
			type: 'POST',
			url: 'include/Libs.php?accion=getCredito',
			dataType:'json',
			data: params,
			error: function(){
				bootbox.dialog({
	 ...
	 * Guardar Gasto
	 */
	$(document).on('submit','#form-salida',function(e){
		e.preventDefault();
		$.ajax({
			type: 'POST',
			url: 'include/Libs.php?accion=saveSalida',
			data: $('form[name="form-salida"]').serialize(),
			dataType:'json',
			beforeSend: function(){
