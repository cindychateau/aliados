...
$(document).ready(function(){
		...
		if(!isEmpty(id_cl)) {
			if(jQuery.inArray(id_cl,ids_clientes) == -1) {
				var row = '<tr id="row_'+clientes+'">'+
							...
							'</td>'+
							'<td align="center">'+
								'<input id="ahorro_d_'+clientes+'" name="ahorro_d['+clientes+']" type="text" class="form-control ahorro_d" data-id="'+clientes+'" readonly="readonly">'+
							'</td>'+
							'<td align="center">'+
	 ...
	 */
	$(document).on('change','#tasa, #ahorro_p, #comision_p, #plazo',function(e) {
		var tasa = $("#tasa").val();
		var ahorro_p = $("#ahorro_p").val();
		...
		$('.monto_individual').each(function(){
			...
			if(!isEmpty(monto_individual) && !isEmpty(tasa) && !isEmpty(ahorro_p) && !isEmpty(comision_p) && !isEmpty(plazo)) {
				...
				var ahorro_d = monto_individual * ahorro_p;
				ahorro_d = Math.round(ahorro_d * 100) / 100;
				$("#ahorro_d_"+id_row).val(ahorro_d);
				...
				var monto_otorgado = monto_individual - comision_d - ahorro_d;
	...
	$.ajax({
		...
		success: function(result){
			if(!result.error) {
				...
				$("#ahorro_p").val(result.GRU_AHORRO_P);
