...
class Libs extends Common {
				...
				try {
					...
					foreach ($result_personas as $row_personas) {
						...
						$json['pagos'] .= '<tr>
												...
												<td align="center">
													<input class="form-control pago dinero_'.$num2.'  pago_'.$row['GRU_ID'].'" type="text" id="pago_'.$row_personas['PER_ID'].'_'.$row['GRU_ID'].'" data-id="'.$row_personas['PER_ID'].'" data-group="'.$row['GRU_ID'].'" data-num="'.$num2.'" name="pago['.$row_personas['PI_ID'].']" value="'.$row_personas['PI_PAGO'].'">
												</td>
												<td align="center">
												...
												<td align="center">
													$<span class="pago_individual_'.$num2.'">
														'.number_format(($row_personas['PI_AHORRO'] + $row_personas['PI_AHORRO']), 2).'
													</span>
												</td>
	...
	function guardarPago() {
		...
		foreach ($_POST['efectuado'] as $key => $value) {
			$sql_tp = "UPDATE TABLA_PAGOS SET TP_EFECTUADO = ?,
											  ...
											  TP_FALTANTE = ?
						WHERE TP_ID = ?";
			$values_tp = array($_POST['efectuado'][$key],
		...
		foreach ($_POST['pago'] as $key => $value) {
			$sql_pi = "UPDATE PAGOS_INDIVIDUALES SET PI_PAGO = ?,
												     ...
												     PI_FECHA_REG = ?
						WHERE PI_ID = ?";
			$values_pi = array($_POST['pago'][$key],
							   $_POST['ahorro'][$key],
							   $_POST['pendiente'][$key],
							   $_POST['comment'][$key],
							   date("Y-m-d"),
			...
			try {
				...
				if ($consulta->rowCount() > 0) {
					...
					foreach ($result as $row) {
						...
						$inpt_pago = '<input type="text" class="form-control pago" name="tpi_efectuado['.$row['TPI_ID'].']" id="tpi_efectuado_'.$row['TPI_ID'].'" value="'.$row['TPI_EFECTUADO'].'" data-id="'.$row['TPI_ID'].'">';
						$inpt_ahorro = '<input type="text" class="form-control ahorro" name="tpi_ahorro['.$row['TPI_ID'].']" id="tpi_ahorro_'.$row['TPI_ID'].'" value="'.$row['TPI_AHORRO'].'" data-id="'.$row['TPI_ID'].'">';
						$inpt_comment = '<textarea class="form-control" id="comentarios_'.$row['TPI_ID'].'" name="comentarios['.$row['TPI_ID'].']" data-id="'.$row['TPI_ID'].'">'.$row['TPI_COMMENT'].'</textarea>';
						...
						$json['pagos'] .= '<tr>
												...
												<td align="center">
													'.$this->printLink($module, "cambios", $inpt_comment).'
												</td>
											</tr>';
					}

					$btn_guardar = '<a class="guardar pull-left" href="#" data-id="">
	...
	function savePagosInd() {
		...
		$db = $this->_conexion;
		$db->beginTransaction(); 
		$sql = "UPDATE TABLA_PAGOS_IND SET TPI_EFECTUADO = ?,
										   TPI_AHORRO = ?,
										   TPI_PAGADO = ?,
										   TPI_FALTANTE = ?
			    WHERE TPI_ID = ? ";
