var App = function () {
    ...
    var currentSlider = 1; // number of sliders
    var sliderTooltip = true; //Show the slider with tooltip
    var minSlider = 1;
    var maxSlider = 100;
    var minSlider1 = 1;
    var maxSlider1 = 100;
	...
	/*	jQuery UI Sliders
	/*-----------------------------------------------------------------------------------*/
	var handleSliders = function (num, mins, maxs, min2, maxs2) {
    ...
    for(var i=0; i<num; i++) {
        if(i==0){
            $("#slider").slider({ value:mins, min: mins, max: maxs, slide: repositionTooltip, stop: repositionTooltip });
            if(sliderTooltip)
                $("#slider .ui-slider-handle:first").tooltip( {title: $("#slider").slider("value"), trigger: "manual"}).tooltip("show");
        } else {
            $("#slider"+i).slider({ value:mins, min: min2, max: maxs2, slide: repositionTooltip, stop: repositionTooltip });
            if(sliderTooltip)
                $("#slider"+i+" .ui-slider-handle:first").tooltip( {title: $("#slider"+i).slider("value"), trigger: "manual"}).tooltip("show");
        }
    }
    ...
    var newSlider = function (num) {
        ...
        $("#slider"+num).slider({ value: 1, slide: repositionTooltip, stop: repositionTooltip });
        $("#slider"+num+" .ui-slider-handle:first").tooltip( {title: $("#slider"+num).slider("value"), trigger: "manual"}).tooltip("show");
    }
	...
	return {
        ...
        init: function () {
			...
			if (App.isPage("sliders_progress")) {
				handleSliders(currentSlider, minSlider, maxSlider, minSlider1, maxSlider1); //Function to display sliders
        ...
        setSliders: function (num) {
            currentSlider = num;
        },

        //Set number of sliders
        setSliderTooltip: function (toolt) {
            sliderTooltip = toolt;
        },

        //Set min number in slider
        setMinSlider: function (min) {
            minSlider = min;
        },

        //Set max number in slider
        setMaxSlider: function (max) {
            maxSlider = max;
        },
         //Set min number in slider
        setMinSlider1: function (min) {
            minSlider1 = min;
        },

        //Set max number in slider
        setMaxSlider1: function (max) {
            maxSlider1 = max;
        },
        //New Slide
        newSlide: function (num) {
            newSlider(num);
        },
