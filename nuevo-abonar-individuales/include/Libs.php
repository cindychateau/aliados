...
class Libs extends Common {
			...
			foreach ($result as $row) {
				...
				try {
					...
					$json['pagos'] .='			</tbody>
											...
											<a class="guardar" href="#" data-id="'.$row['GRU_ID'].'"><button class="btn btn-info"><i class="fa fa-save"></i>Guardar Pagos Individual '.$row['GRU_ID'].'</button></a>
											</div>
										</div>
									</div>';
	...
	function getTotales() {
		...
		try {
			...
			if($_SESSION["mp"]["userprofile"] != 3 && $_SESSION["mp"]["userprofile"] != 5) {
				$sql = "SELECT SUM(TPI_MONTO) as monto, 
							   CREDITO_INDIVIDUAL.SIU_ID,
							   SIU_NOMBRE
						FROM TABLA_PAGOS_IND
						JOIN CREDITO_INDIVIDUAL ON CREDITO_INDIVIDUAL.CRE_ID = TABLA_PAGOS_IND.CRE_ID
						JOIN SISTEMA_USUARIO ON SISTEMA_USUARIO.SIU_ID = CREDITO_INDIVIDUAL.SIU_ID
						WHERE TPI_FECHA >= ?
						AND TPI_FECHA <= ?
                        GROUP BY CREDITO_INDIVIDUAL.SIU_ID
						ORDER BY SIU_NOMBRE, TPI_FECHA, TABLA_PAGOS_IND.CRE_ID";
				...
				try {
					...
					foreach ($result as $row) {
						...
						$sql_desglosado = "SELECT PDI_FECHA,
												  SUM(PDI_MONTO) as entregado,
												  PAGOS_DESGLOSADOS_IND.CRE_ID,
												  SIU_ID
											FROM PAGOS_DESGLOSADOS_IND
											JOIN CREDITO_INDIVIDUAL ON CREDITO_INDIVIDUAL.CRE_ID = PAGOS_DESGLOSADOS_IND.CRE_ID
											WHERE SIU_ID = ?
											AND TPI_ID IN
												(SELECT TPI_ID 
                                                 FROM TABLA_PAGOS_IND
						...
						$sql_recuperado = "SELECT PRI_FECHA,
												  SUM(PRI_MONTO) as recuperado,
												  PAGOS_RECUPERADOS_IND.CRE_ID,
												  SIU_ID
											FROM PAGOS_RECUPERADOS_IND
											JOIN CREDITO_INDIVIDUAL ON CREDITO_INDIVIDUAL.CRE_ID = PAGOS_RECUPERADOS_IND.CRE_ID
											WHERE SIU_ID = ?
											AND PRI_FECHA >= ?
											AND PRI_FECHA <= ?";
	...
	function filtrarCreds() {
		...
		if(isset($_POST['promotor'])) {
			...
			//TOTAL de grupos
			$sql_num = 'SELECT COUNT(CRE_ID) as Total_Registros
						FROM CREDITO_INDIVIDUAL WHERE 1 = 1 ';
			$values_num = array();			

			if($_POST['promotor'] > 0) {
				$sql_num .= ' AND SIU_ID = ? ';
				$values_num[]= $_POST['promotor'];
			}

			if($_POST['activo_inac'] != 2) {
				$sql_num .= ' AND CRE_VIGENTE = ? ';
				$values_num[]= $_POST['activo_inac'];
			}
			...
			$sql_grupos = "SELECT CRE_ID,
							...
							WHERE 1 = 1
							".($_POST['promotor'] > 0 ? 'AND SIU_ID = ?' : '')."
							".($_POST['activo_inac'] != 2 ? 'AND CRE_VIGENTE = ?' : '')."
							ORDER BY CRE_ID DESC ".$limit;

			$values_grupos = array();

			if($_POST['promotor'] > 0) {
				$values_grupos[]= $_POST['promotor'];
			}

			if($_POST['activo_inac'] != 2) {
				$values_grupos[]=$_POST['activo_inac'];
			}
			...
			if($_POST['promotor'] > 0) {
				$sql_promotor_total = "SELECT SUM(CRE_MONTO_TOTAL) as total
									   FROM CREDITO_INDIVIDUAL
									   WHERE SIU_ID = ?";
				...
				$sql_promotor_pend = "SELECT SUM(TPI_FALTANTE) as pendiente
								 	  FROM TABLA_PAGOS_IND
								 	  JOIN CREDITO_INDIVIDUAL ON CREDITO_INDIVIDUAL.CRE_ID = TABLA_PAGOS_IND.CRE_ID
								 	  WHERE SIU_ID = ?
				...
				$json['pagos'] = '<table class="table">
									<thead>
										<tr>
											<th>Colocación Total Histórica (Ind)</th>
			...
			try {
				...
				if ($consulta_grupos->rowCount()) {
					...
					foreach ($result_grupos as $row) {
						...
						$sql_tabla_pagos = "SELECT TPI_FECHA
											FROM TABLA_PAGOS_IND
											WHERE CRE_ID = ?
											ORDER BY TPI_FECHA ASC";
						$value_tabla_pagos = array($row['CRE_ID']);	
						...
						$json['pagos'] .= '<div class="col-md-12 div-'.$row['CRE_ID'].'">
											<form class="form-'.$row['CRE_ID'].'">
											<div class="box border '.$color.'">
												<input type="hidden" class="form-control" id="cre_'.$row['CRE_ID'].'" name="cre_id" value="'.$row['CRE_ID'].'">
												<div class="box-title">
												...
												<div class="box-body" style="display:none;">
													...
													<table class="table table-striped general-info" data-id="'.$row['CRE_ID'].'">
														<tbody>
															<tr>
																<td align="center"><b>Fecha de Apertura:</b> '.date("d/m/Y",strtotime($row["CRE_FECHA_ENTREGA"])).'</td>
																<td align="center"><b>Fecha de Vencimiento:</b> '.date("d/m/Y",strtotime($row["CRE_FECHA_FINAL"])).'</td>
															</tr>
														  	<tr>
																<td align="center"><b>Crédito Grupal Autorizado: </b> $'.number_format($row['CRE_MONTO_TOTAL'], 2).'</td>
																<td align="center"><b>Pago Semanal Grupal: </b> $'.number_format($row['CRE_PAGO_SEMANAL'], 2).'</td>
															</tr>
														</tbody>
													  </table>
						...
						foreach ($result_tabla_pagos as $row_tabla_pagos) {
							$num_pagos++;
							$json['pagos'] .= '<th><span class="pop-hover" data-content="'.(isset($row_tabla_pagos['TPI_FECHA']) ? date("d/m/Y",strtotime($row_tabla_pagos["TPI_FECHA"])) : '-').'">'.$num_pagos.'</span></th>';
						}

						if(count($result_tabla_pagos) < 12) {
							$i_x = count($result_tabla_pagos) +1;
							for ($i=$i_x; $i < ($row['CRE_PLAZO'] + 1); $i++) { 
					...
					$sql_personas = "SELECT CONCAT(PER_NOMBRE, ' ', PER_APELLIDO_PAT, ' ', PER_APELLIDO_MAT) as PER_NOMBRE,
								 	...
								 	WHERE CRE_ID = ?";
					$values_personas = array($row['CRE_ID']);
					$consulta_personas = $db->prepare($sql_personas);
					$pagos_arr = array();
					
					$pagos_arr = array_fill(0, ($row['CRE_PLAZO']+1), 0);
					...
					try {
						...
						foreach ($result_personas as $row_personas) {
							$json['pagos'] .= '<tr>
												<td align="center" rowspan="2">'.$row_personas['PER_NOMBRE'].'</td>
												<td align="center" rowspan="2">'
													.$row['CRE_PAGO_SEMANAL'].'
													<input type="hidden" class="form-control pago_semanal_ind" id="pago_semanal_ind_'.$row_personas['PER_ID'].'" value="'.$row['CRE_PAGO_SEMANAL'].'" data-id='.$row_personas['PER_ID'].'>
												</td>
												<td align="center">P</td>';

							$pagos_arr[0] += $row['CRE_PAGO_SEMANAL'];					

							$sql_pagos = "SELECT * FROM TABLA_PAGOS_IND
										  WHERE CRE_ID = ?
										  ORDER BY TPI_FECHA ASC";
							$values_pagos = array($row['CRE_ID']);
							...
							try {
								...
								foreach ($result_pagos as $row_pagos) {
									...
									$pago = ($row_pagos['TPI_REC'] == 0 ? $row_pagos['TPI_PAGADO'] : 0);

									$sql_desglosado = "SELECT * FROM
														(
															(SELECT PDI_FECHA as FECHA,
																	PDI_MONTO as MONTO,
																	'D' as DoR,
																	'0' as ADI_ID,
																	PDI_ID as ID_D
														...
														PAGOS
														ORDER BY FECHA ASC";
									$values_desglosado = array($row_pagos['TPI_ID'],
															   $row_pagos['TPI_ID']);
									...
									try {
										...
										foreach ($result_desglosado as $row_desglosado) {
											$hover_pago .= "<span class='".($row_desglosado['DoR'] == 'R' ? ($row_desglosado['ADI_ID'] > 0 ? 'text-purple' : 'text-muted') : '')."'>".date("d/m/Y",strtotime($row_desglosado["FECHA"]))." - ".$row_desglosado['MONTO']."</span><br>";
									...
									$color = ($row_pagos['TPI_PAGADO'] > 0 ? ($row_pagos['TPI_PAGADO'] == $row_pagos['TPI_MONTO'] ? 'text-success' : 'text-warning') : '');

									$color = ($row_pagos['TPI_REC'] == 2 ? 'text-info' : $color);


									$json['pagos'] .= '<td align="center"><span class="pop-hover '.$color.'" data-html="true" data-content="'.$hover_pago.'">'.$pago.'</span></td>';
									$row_ahorro .= '<td align="center">'.$row_pagos['TPI_AHORRO'].'</td>';
									$total_abonado += $pago;
									$totales_abonado += $pago;
									$suma_ahorro += $row_pagos['TPI_AHORRO'];
									$totales_ahorrado += $row_pagos['TPI_AHORRO'];
									$pagos_arr[$p] += $pago;
							...
							$sql_rec = "SELECT * FROM PAGOS_RECUPERADOS_IND
										WHERE CRE_ID = ?";
							$values_rec = array($row['CRE_ID']);	
							...
							try {
								...
								foreach ($result_rec as $row_rec) {
									$hover_rec .= "<span class='".($row_rec['ADI_ID'] > 0 ? 'text-purple' : '')."'>".date("d/m/Y",strtotime($row_rec["PRI_FECHA"]))." - ".$row_rec['PRI_MONTO']."<br>";
									$recuperados += $row_rec['PRI_MONTO'];
									$totales_recuperado += $row_rec['PRI_MONTO'];
								}
							...
							//Deuda REAL
							$sql_deuda = "SELECT SUM(TPI_FALTANTE) as deuda
										  FROM TABLA_PAGOS_IND 
										  WHERE CRE_ID = ?
										  AND TPI_FECHA < CURRENT_DATE";
							$values_deuda = array($row['CRE_ID']);	
							...
							$sql_ad = "SELECT * FROM AHORROS_DESGLOSADOS_IND
									   WHERE CRE_ID = ?";
							$values_ad = array($row['CRE_ID']);	
							...
							try {
								...
								foreach ($result_ad as $row_ad) {
									$hover_ad .= "<span class='".($row_ad['ADI_VALIDO'] == 0 ? 'crossed' : '')."'>".date("d/m/Y",strtotime($row_ad["ADI_FECHA"]))." - ".$row_ad['ADI_CANTIDAD']."</span><br>";
								}
							...
							$trans_ahorro = '<span class="fa-stack pop-hover transferir-ahorro" data-content="Transferir Ahorro" data-per="'.$row_personas['PER_ID'].'" data-gru="'.$row['CRE_ID'].'">
					...
					for ($i=1; $i < $row['CRE_PLAZO']+1; $i++) { 
						$json['pagos'].= 						'<td align="center">'.$pagos_arr[$i].'</td>';
					}

					$btn_guardar = '<a class="guardar" href="#" data-id="'.$row['CRE_ID'].'">
														<button class="btn btn-success"><i class="fa fa-save"></i>Guardar Pagos Individual '.$row['CRE_ID'].'</button>
													</a>';
					$btn_autocomplete = '<a class="autocomplete" href="#" data-id="'.$row['CRE_ID'].'">
					...
					$json['pagos'] = "<h1>No se encontraron créditos</h1>";
				}
	...
	function filterClients() {
		...
		if(isset($_POST['cliente']) && $_POST['cliente'] > 0) {
			...
			$sql_grupos = "SELECT CRE_ID,
								  ...
								  SIU_NOMBRE
							FROM CREDITO_INDIVIDUAL
							LEFT JOIN SISTEMA_USUARIO ON SISTEMA_USUARIO.SIU_ID = CREDITO_INDIVIDUAL.SIU_ID
							WHERE PER_ID = ?
	...
	function fillPagosDesglosados() {
		...
		try {
			...
			foreach ($result as $row) {
				...
				$sql_ind = "INSERT INTO PAGOS_DESGLOSADOS_IND (PDI_FECHA,
															   PDI_MONTO,
															   PDI_AHORRO,
															   CRE_ID,
															   TPI_ID)
						  	VALUES (?, ?, ?, ?, ? )";
				$values_ind = array($row['TPI_FECHA'],
									$row['TPI_PAGADO'],
									$row['TPI_AHORRO'],
									$row['CRE_ID'],
									$row['TPI_ID']);		
				...
				$sql_ind = "UPDATE TABLA_PAGOS_IND SET TPI_FECHA_REG = ?
							WHERE TPI_ID = ?";
				$values_ind = array($row['TPI_FECHA'],
									$row['TPI_ID']);		
				$consulta_ind = $db->prepare($sql_ind);	
	...
	function savePagos() {
		$json = array();
		$json['error'] = false;
		$json['msg'] = "El Crédito se guardó con éxito.";
		$msg_extra = "";
		...
		$sql = "SELECT TABLA_PAGOS_IND.*, 
					   PER_ID  
				FROM TABLA_PAGOS_IND
				JOIN CREDITO_INDIVIDUAL ON TABLA_PAGOS_IND.CRE_ID = CREDITO_INDIVIDUAL.CRE_ID
				WHERE TABLA_PAGOS_IND.CRE_ID = ?
				AND TPI_FECHA >= ?
				AND TPI_FECHA <= ?";

		$values = array($_POST['cre_id'],
						$fecha_inicio,
						$fecha_fin);
		$consulta = $db->prepare($sql);
		...
		try {
			...
			if($consulta->rowCount()) {
				foreach ($result as $row) {
					...
					if($row['TPI_FALTANTE'] > 0 && $pago > 0 && $pago != '') {
						$deuda = $pago - $pendiente;
						$ahorro = $row['TPI_AHORRO'] + $_POST['ahorro'][$per_id];

						if($deuda == 0 || $deuda > 0) {
							$pago_sql = $row['TPI_MONTO'];
							$pendiente = 0;
							$pago = $deuda;
						} else if($deuda < 0) {
							$pago_sql = $row['TPI_PAGADO'] + $pago;
							$pendiente = abs($deuda);
							$pago = 0;
						}
						...
						$sql_pi = "UPDATE TABLA_PAGOS_IND SET TPI_PAGADO = ?,
															  TPI_AHORRO = ?,
															  TPI_FALTANTE = ?,
															  TPI_FECHA_REG = ?
									WHERE TPI_ID = ?";
						...
						$sql_pd = "INSERT INTO PAGOS_DESGLOSADOS_IND (PDI_FECHA,
																	  PDI_MONTO,
																	  PDI_AHORRO,
																	  CRE_ID,
																	  TPI_ID)
					...
					if(!$ahorro_registrado) {
						...
						//Hace UPDATE del Pago Individual
						$sql_pi = "UPDATE TABLA_PAGOS_IND SET TPI_AHORRO = ?
									WHERE TPI_ID = ?";
						...
						if($_POST['ahorro'][$per_id] > 0 && $_POST['ahorro'][$per_id] != '') {
							$sql_ad = "INSERT INTO AHORROS_DESGLOSADOS_IND (ADI_FECHA,
																		  	CRE_ID,
																		  	TPI_ID,
																		  	ADI_CANTIDAD)
									  	VALUES ( ?, ?, ?, ?)";
					...
					if($pago > 0) {
						...
						$sql_pendientes = "SELECT * FROM TABLA_PAGOS_IND
										   WHERE TPI_FALTANTE > 0
										   AND CRE_ID = ?
										   ORDER BY TPI_FECHA ASC";
						$values_pendientes = array($_POST['cre_id']);
						$consulta_pendientes = $db->prepare($sql_pendientes);	
						...
						try{
							...
							if(count($result_pendientes)) {
								while ($pago > 0 && count($result_pendientes) > $n) {
									...
									$pendiente = $row_pendiente['TPI_FALTANTE'];

									$deuda = $pago - $pendiente;
									if($deuda == 0 || $deuda > 0) {
										$pago_sql = $row_pendiente['TPI_MONTO'];
										$pendiente = 0;
										$nuevo_pago = $deuda;
									} else if($deuda < 0) {
										$pago_sql = $row_pendiente['TPI_PAGADO'] + $pago;
										$pendiente = abs($deuda);
										$nuevo_pago = 0;
									}
									...
									$rec = (strtotime($fecha_actual) < strtotime($row_pendiente['TPI_FECHA']) ? 0 : 1);
									...
									if(!$ahorro_registrado) {
										...
										$ahorro = $row_pendiente['TPI_AHORRO'] + $_POST['ahorro'][$per_id];			
									...
									//Si es mayor a la fecha actual = Desglosado
									if($rec == 1) {
										$sql_pd = "INSERT INTO PAGOS_RECUPERADOS_IND (PRI_FECHA,
																					  PRI_MONTO,
																					  CRE_ID,
																					  TPI_ID)
												  	VALUES ( ?, ?, ?, ? )";
				...
				foreach ($_POST['pago'] as $per_id => $pago) {
							...
							if(count($result_pendientes)) {
								while ($pago > 0 && count($result_pendientes) > $n) {
									...
									try{
										...
										$db->rollBack();
										die($e->getMessage().$row_pendiente['TPI_ID']);
										$json['msg'] = "Experimentamos Fallas Técnicas. Comuníquese con su proveedor.";
									}
					...
					if(!$ahorro_registrado) {
						...
						$sql_pi_a = "SELECT * FROM TABLA_PAGOS_IND
								     WHERE TPI_FECHA < CURRENT_DATE
								     AND CRE_ID = ?
								     ORDER BY TPI_FECHA DESC";
						$values_pi_a = array($_POST['cre_id']);
						...
						$ahorro = $row_pi_a['TPI_AHORRO'] + $_POST['ahorro'][$per_id];
	...
	function reloadCred() {
		...
		if(isset($_POST['cre_id'])) {
			...
			$values_grupos = array($_POST['cre_id']);
			$consulta_grupos = $db->prepare($sql_grupos);
			...
			try {
				...
				if ($consulta_grupos->rowCount()) {
					...
					//foreach ($result_grupos as $row) {
						...
						$json['pagos'] .= '<table class="table table-striped general-info" data-id="'.$row['CRE_ID'].'">
														<tbody>
															<tr>
																<td align="center"><b>Fecha de Apertura:</b> '.date("d/m/Y",strtotime($row["CRE_FECHA_ENTREGA"])).'</td>
																<td align="center"><b>Fecha de Vencimiento:</b> '.date("d/m/Y",strtotime($row["CRE_FECHA_FINAL"])).'</td>
					...
					$select = '<div class="cont-sel-'.$row['CRE_ID'].' display-none">';
					...
					try {
						...
						foreach ($result_personas as $row_personas) {
							...
							try {
								...
								$recuperados = 0;
								foreach ($result_rec as $row_rec) {
									$hover_rec .= date("d/m/Y",strtotime($row_rec["PRI_FECHA"]))." - ".$row_rec['PRI_MONTO']."<br>";
									$recuperados += $row_rec['PRI_MONTO'];
									$totales_recuperado += $row_rec['PRI_MONTO'];
								}
	...
	function filterByCred() {
		...
		if(isset($_POST['cre_id']) && $_POST['cre_id'] > 0) {
