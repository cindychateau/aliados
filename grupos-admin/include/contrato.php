...
if (isset($_GET['id'])) {
	...
	class PDF extends PDF_Rotate{
		...
		function Header() {
			...
			if ($this->head == 1)
				$this->Cell(180, 5,"FOLIO G".$id_fill,0,0,"R",0);
			else if($this->head == 3)
				$this->Cell(270, 5,"FOLIO G".$id_fill,0,0,"R",0);
	...
	$mes_act = date("m");
	$anio_actual = date("Y");*/
	$dia_actual = date("d",strtotime($grupo["GRU_FECHA_ENTREGA"]));
	$mes_act = date("m",strtotime($grupo["GRU_FECHA_ENTREGA"]));
	$anio_actual = date("Y",strtotime($grupo["GRU_FECHA_ENTREGA"]));
	...
	if($grupo['GRU_DOMICILIO'] == "")
		$grupo['GRU_DOMICILIO'] = "la calle _______, número____, Colonia _____, Código Postal____, de la Ciudad de Monterrey, Estado de Nuevo León";

	$i3 = utf8_decode("I.3     Señala como domicilio para oír notificaciones y recibir toda clase de documentos, el inmueble ubicado en ".$grupo['GRU_DOMICILIO'].".");
	...
	$i422 = utf8_decode("II.2     Señalan como domicilio común para oír notificaciones y recibir toda clase de documentos, el inmueble ubicado en ".$grupo["GRU_DOMICILIO"].".");	
	$pdf->Ln(3);
	$pdf->Write(5, $i422);
	...
	$i433 = utf8_decode("III.3     Tener la capacidad juridica y legal suficiente para obligarse, en los términos del presente contrato, lo cual al efecto no se podrá alegar nulidad del mismo, por inexperiencia o ignorancia.");	
	$pdf->Ln(8);
	$pdf->Write(5, $i433);
	...
	$equivalente = ($grupo['GRU_RECREDITO'] == 0 ? 5 : 2.5);


	$txt33 = utf8_decode("          Previo al retiro del importe del crédito concedido, las Acreditadas deberán realizar el pago de la comisión por apertura, la cual equivale al ".$equivalente."% del valor del crédito concedido.");
	$pdf->Ln(8);
	$pdf->SetFont('Arial','',8);
	$pdf->Write(5, $txt31);
	$pdf->Ln(8);
	...
	$txt31 = utf8_decode("          Las Acreditadas se obligan a pagar al Acreditante en un plazo de ".$grupo['GRU_PLAZO']." semanas contados a partir de la firma del presente contrato, la totalidad del monto del crédito solicitado, cuya cantidad asciende a $".$monto_bonito);
	...
	$pdf->SetFont('Arial','U',8);
	$pdf->Write(5, utf8_decode($grupo['GRU_DOMICILIO']).".");
	...
	$tasa_palabras = $NumberToLetterConverter->to_word($tasa_bonito, null) ."POR CIENTO";
	$pdf->Ln(8);
	$pdf->SetFont('Arial','',8);
	$pdf->Write(5, $txt31);
	$pdf->SetFont('Arial','I',8);
...
San Pedro Garza García, incluyendo de manera enunciativa más no limitativa cualquier contrademanda o procedimiento judicial o extrajudicial que se inicie con motivo de la celebración, interpretación y cumplimiento del Contrato, renunciando en este momento al fuero que pudiere corresponderles en razón de su domicilio presente o futuro o por cualquier otra causa.");
	...
	$pdf->Ln(6);
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(0, 4, utf8_decode("TABLA DE PAGOS INDIVIDUALES"), 0, 1, 'C');
	$pdf->Ln(6);
	$pdf->SetFont('Arial','B',8);
	/*$pdf->Cell(63, 7, utf8_decode("NOMBRE"), "LTR", 0, "C");
	$pdf->Cell(63, 7, utf8_decode("MONTO INDIVIDUAL"), "LTR", 0, "C");
	$pdf->Cell(63, 7, utf8_decode("PAGO SEMANAL"), "LTR", 0, "C");
	$pdf->SetFont('Arial','',8);
	$pdf->Ln();
	...
	foreach ($personas as $persona) {
		$border = "LTR";
		if($index == (count($personas)))
			$border = 1;
		$pdf->Cell(63, 7, utf8_decode($persona['PER_NOMBRE']), $border, 0, "C");
		$monto = number_format($persona['MONTO_INDIVIDUAL'], 2);
		$pdf->Cell(63, 7, $monto, $border, 0, "C");
		$monto = number_format($persona['PAGO_SEMANAL_IND'], 2);
		$pdf->Cell(63, 7, $monto, $border, 0, "C");
		$index++;
		$pdf->Ln();
	}*/

	$pdf->SetWidths(array(50,25,53,30,28));
	$pdf->SetAligns(array("C","C","C","C","C"));
	$pdf->Row(array("NOMBRE", "TELEFONO", "DIRECCION", "MONTO INDIVIDUAL", "PAGO SEMANAL"));
	$pdf->SetFont('Arial','',8);
	foreach ($personas as $persona) {
		$nombre = utf8_decode($persona['PER_NOMBRE']);
		$telefonos = $persona['PER_TELEFONO']." ".$persona['PER_CELULAR'];
		...
		$monto = number_format($persona['MONTO_INDIVIDUAL'], 2);
		$pago = number_format($persona['PAGO_SEMANAL_IND'], 2);
		$pdf->Row(array($nombre, $telefonos, $direccion, $monto, $pago));
	}
	...
	$pdf->Ln(6);
	$pdf->SetFont('Arial','B',8);
	$pdf->Cell(0, 4, utf8_decode("CONTROL INTERNO"), 0, 1, 'C');
	$pdf->Ln(6);
	$pdf->SetFont('Arial','B',8);

	//Si el plazo es de 13 semanas o menos UNA hoja de C.I.
	if($grupo['GRU_PLAZO'] < 14) {
		//#
		$pdf->Cell(25, 7, "", "LTR", 0, "C");
		$index = 1;
		$size = 252 / $grupo['GRU_PLAZO'];
		$size = number_format($size, 2);
		foreach ($pagos as $pago) {
			$border = "LTR";
			$pdf->Cell($size, 7, $index, $border, 0, "C");
			$index++;
		}
		$pdf->Ln();
		...
		foreach ($pagos as $pago) {
			$border = "LTR";
			$fecha = date("d/m/Y", strtotime($pago['TP_FECHA']));
			$pdf->Cell($size, 7, $fecha, $border, 0, "C");
			$index++;
		}
		$pdf->Ln();
		...
		$index = 1;
		$mini = $size / 2;
		$mini = number_format($mini, 2);
		foreach ($pagos as $pago) {
			$border = "LTR";
			$pdf->Cell($mini, 7, "P", $border, 0, "C");
			$pdf->Cell($mini, 7, "A", $border, 0, "C");
			$index++;
		}
		$pdf->Ln();
		...
		$array_widths = array_fill(0, (($grupo['GRU_PLAZO']*2) +1), $mini);
		$array_widths[0] = 25;

		$pdf->SetWidths($array_widths);
		$pdf->SetAligns(array("C","C","C","C","C"));
		foreach ($personas as $persona) {
			$nombre = utf8_decode($persona['PER_NOM']." ".$persona['PER_AP_PAT']);
			$array_row = array_fill(0, (($grupo['GRU_PLAZO']*2) +1), "");
			$array_row[0] = $nombre;
			$pdf->Row($array_row);
		}
	} else {
		//Dividimos el C.I. en dos

		//PRIMERA PÁGINA
		$columnas = round(($grupo['GRU_PLAZO'] / 2), 0, PHP_ROUND_HALF_UP);
		$pdf->Cell(25, 7, "", "LTR", 0, "C");
		$size = 252 / $columnas;
		$size = number_format($size, 2);
		//foreach ($pagos as $pago) {
		...
		for ($i=0; $i < ($columnas); $i++) { 
			$border = "LTR";
			$fecha = date("d/m/Y", strtotime($pagos[$i]['TP_FECHA']));
			$pdf->Cell($size, 7, $fecha, $border, 0, "C");
		}
		$pdf->Ln();
		...
		foreach ($personas as $persona) {
			$nombre = utf8_decode($persona['PER_NOM']." ".$persona['PER_AP_PAT']);
			$array_row = array_fill(0, (($columnas*2) +1), "");
			$array_row[0] = $nombre;
			$pdf->Row($array_row);
		}
		...
		$columnas_old = $columnas;
		$columnas = $grupo['GRU_PLAZO'] - $columnas_old;
		$pdf->Cell(25, 7, '', "LTR", 0, "C");
		$size = 252 / $columnas;
		$size = number_format($size, 2);
		//foreach ($pagos as $pago) {
		for ($i=($columnas_old+1); $i < ($grupo['GRU_PLAZO']+1); $i++) { 
			$border = "LTR";
			$pdf->Cell($size, 7, $i, $border, 0, "C");
		}
		$pdf->Ln();
		...
		$pdf->Cell(25, 7, "", "LTR", 0, "C");
		//foreach ($pagos as $pago) {
		for ($i=$columnas_old; $i < $grupo['GRU_PLAZO']; $i++) { 
			$border = "LTR";
			$fecha = date("d/m/Y", strtotime($pagos[$i]['TP_FECHA']));
			$pdf->Cell($size, 7, $fecha, $border, 0, "C");
		}
	...
	foreach ($personas as $clave => $persona) {
		...
		if($clave == ($num_personas - 1)) {
			$aval = $personas[0];
		} else {
			$aval = $personas[$clave+1];
		}

		$aval_dir = utf8_decode($aval['PER_DIRECCION']);
		if($aval['PER_NUM'] != '') {
			$aval_dir.= utf8_decode(" No. ".($aval['PER_NUM']).", ".($aval['PER_COLONIA']).", ".($aval['PER_MUNICIPIO']).", ".($aval['PER_ESTADO']).", ".($aval['PER_CP']));
		}

		$aval = utf8_decode($aval['PER_NOMBRE']);

		$pago_bonito = number_format($persona['MONTO_INDIVIDUAL'], 2);
		$pago_letras = $pdf->numtoletras($pago_bonito);
		$dia_pago = date("d", strtotime($pagos[11]['TP_FECHA']));
		$mes_pag = date("m", strtotime($pagos[11]['TP_FECHA']));
		$mes_pago = $pdf->mes($mes_pag);
		$anio_pago = date("Y", strtotime($pagos[11]['TP_FECHA']));
		$pdf->head = 2;
		if($num == 1) {
			$pdf->AddPage();
			$num++;
		...
		$pdf->Write(6, strtoupper($txt_pagare));
		...
		$pdf->Write(6, strtoupper("GRUPO PROPULSOR DE MICROEMPRESAS DEL NORTE S.A.P.I. DE C.V."));
		$txt_pagare = utf8_decode("., en esta plaza el dÍa ".$dia_pago." de ".$mes_pago." del ".$anio_pago." en el domicilio ubicacado en ".$grupo['SIU_DIRECCION'].", la cantidad de: $".$pago_bonito." (".$pago_letras."), el dÍa de su pago. Este documento causarÁ un interÉs del ".($tasa_bonito*4)."% mensual.");
		$pdf->SetFont('Arial','',7);
		$pdf->Write(6, strtoupper($txt_pagare));
		$pdf->Ln();
		...
		$sus_y = $pdf->GetY();
		$pdf->Write(6, strtoupper("Suscriptor:"));
		...
		//	$persona = $personas[0];
			...
			//if($c) {
				$current_y = $pdf->GetY();
				$pdf->MultiCell(75, 6,$txt_per.$direccion,"T","C",0);
				$pdf->SetY($sus_y);
		...
		$pdf->SetFont('Arial','BU',7);
		$pdf->SetX(115);
		$pdf->Write(6, strtoupper("Avales:"));
		$pdf->SetFont('Arial','',7);
		$pdf->Ln();
		$pdf->Ln();
		$pdf->Ln();
		$pdf->SetX(115);
		$pdf->MultiCell(75, 6,$aval."\n".$aval_dir,"T","C",0);	
		$pdf->Ln();	
