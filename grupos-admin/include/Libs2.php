					...
					if(count($grupos_rec) > 0) {
						...
						/*foreach ($grupos_rec as $grupo_rec) {
							...
							if(is_null($result_cant['NUEVO_PAGO']) OR $result_cant['NUEVO_PAGO'] == 0) {
								...
								$sql_nuevo = "DELETE FROM TABLA_PAGOS
											  WHERE TP_FECHA >= ?
											  AND GRU_ID = ?";
								...
								$sql_nuevo = "UPDATE TABLA_PAGOS SET TP_MONTO = ?,
																	 TP_FALTANTE = ?
											  WHERE TP_FECHA >= ?
											  AND GRU_ID = ?";
						...
						foreach ($_POST['cli_id'] as $num => $id_cliente) {
							...
							//Elimina los PAGOS_INDIVIDUALES de la persona que se recreditó
							/*try{
								$sql_pi = "DELETE FROM PAGOS_INDIVIDUALES 
										   WHERE PER_ID = ?
										   AND PI_FECHA >= ?
										   AND (";
							...
							try{
								$sql_pi = "UPDATE PAGOS_INDIVIDUALES SET PI_PAGO = PI_MONTO,
																		 PI_PENDIENTE = 0,
																		 PI_FECHA_REG = ?,
																		 PI_REC = 2
										   WHERE PER_ID = ?
													...
													<tbody>
														<tr>
													  	...
													  	<tr>
															...
															<td align="center"><a href="grupos.php?id='.$row['SIU_ID'].'">'.$row['SIU_NOMBRE'].'</a> <a href="#" class="editar-promotor" data-id="'.$row['GRU_ID'].'"><i class="fa fa-pencil"></i></a></td>
													  	</tr>
													  	'.($row['GRU_RECREDITO'] != 0 ?
										...
										<div class="box border blue">
											<div class="box-title">
												...
												<div class="tools">
													...
													<a href="cambios-ind.php?id='.$row['CRE_ID'].'" class="edit" data-id="'.$row['CRE_ID'].'">
														<i class="fa fa-pencil"></i>
													</a>
												</div>
											</div>
		...
		if($_POST['tipo'] == 1 || $_POST['tipo'] == 2 || $_POST['tipo'] == 0 || $_POST['tipo'] == 4) {
			try {
				...
				$sql = "SELECT GRU_ID,
						...
						FROM GRUPOS 
						JOIN SISTEMA_USUARIO ON GRUPOS.SIU_ID = SISTEMA_USUARIO.SIU_ID
						WHERE GRU_VIGENTE = 1".$promotor." ".$tipo." ".$fechas."
						ORDER BY GRU_FECHA DESC, GRU_ID DESC";
				//die($sql);		
				$consulta = $db->prepare($sql);
				$consulta->execute();
